...
class Kinopoisk_testing_ui:
        ...
        try:
            search_button_svg = WebDriverWait(self._driver, 10).until(
                EC.presence_of_element_located((
                    By.XPATH,
                    '//button[contains(@class, "styles_submit__2AIpj") and @aria-label="Найти"]'
                ))
            )
    ...
    def extended_search_movie(self, movie_name: str = None, year: int = None,
                              country: str = None, actor: str = None,
                              genre: str = None):
        """
        Запрос с расширенными параметрами поиска: название, год, страна, актер и жанр.
        Если параметр не указан, берутся значения из конфигурации.
        """
        if not movie_name:
        ...
        try:
            wait.until(
                EC.visibility_of_element_located((By.CSS_SELECTOR, 'a[aria-label="Расширенный поиск"]'))
            ).click()

            input_movie_name = wait.until(
                EC.visibility_of_element_located((By.CSS_SELECTOR, '#find_film'))
            )
            ...
            input_country = wait.until(
                EC.visibility_of_element_located((By.CSS_SELECTOR, '#country'))
            )
            input_country.send_keys(country)

            input_actor = wait.until(
                EC.visibility_of_element_located((By.CSS_SELECTOR, 'input[name="m_act[actor]"]'))
            )
            input_actor.send_keys(actor)

            genre_dropdown = wait.until(
                EC.visibility_of_element_located((By.CSS_SELECTOR, 'id="m_act[genre]"'))
            )
    ...
    def open_reviews(self, movie_name: str):
        """
        Находит фильм по названию на главной странице и открывает страницу рецензий зрителей.
        """
        driver = self._driver
        ...
        movie_link = wait.until(
            EC.element_to_be_clickable((By.XPATH, f'//a[text()="{movie_name}"]'))
        )
        movie_link.click()
        wait.until(
            EC.presence_of_element_located((
        ...
        reviews_link = wait.until(
            EC.presence_of_element_located((By.LINK_TEXT, "Рецензии зрителей"))
        )
        reviews_link.click()

    def open_filmography(self, actor: str):
        """
        Находит актера по имени на главной странице и открывает его фильмографию.
        """
        driver = self._driver
        wait = WebDriverWait(driver, 10)
        ...
        actor_link = wait.until(
            EC.element_to_be_clickable((By.XPATH, f'//a[text()="{actor}"]'))
        )
        actor_link.click()
